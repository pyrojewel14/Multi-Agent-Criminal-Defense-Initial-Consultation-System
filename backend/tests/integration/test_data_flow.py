...
assert applied_laws[0]["elements_missing"] == ["轻伤以上后果"]
assert applied_laws[2]["required_elements"] == []
...
mixed_applied_laws = [
    {
        ...
        "required_elements": ["故意", "伤害行为", "轻伤以上后果"],
        "base_sentence": "三年以下有期徒刑",
        "data_source": "rag_verified",
    },
    {
...
assert result_b["source"] == "rag_unverified", f"应返回 rag_unverified，实际为 {result_b['source']}"
assert result_b["degraded"] is True
print("  [PASS] 场景 B: 全部 rag_unverified 时正确返回 degraded")
